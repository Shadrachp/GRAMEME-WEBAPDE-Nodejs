...
const mapRequest = module.exports.mapRequest = function (req) {
...
const stripValues = new Set(['password', 'secret', 'passwd', 'authorization', 'api_key', 'apikey', 'access_token']);
const creditCard = /^(?:\d[ -]*?){13,16}$/; // Regexp to detect credit card numbers
const MAX_ROUNDS = 10;
...
const safe = function (data, i) {
    ...
    Object.keys(data)
        .forEach((key) => {
            ...
            if ((stripValues.has(key)) ||
                (typeof data[key] === 'string' && data[key].match(creditCard) !== null)) {
                data[key] = SAFETY;
                return;
            }
...
module.exports.mapRequestAndArrayHeaders = function (req) {

    const res = safe(mapRequest(req), 0);
    const heads = [];
