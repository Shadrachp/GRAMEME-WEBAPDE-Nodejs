...
const buildModuleDate = module.exports._buildModuleData = function (pathToModule) {
    ...
    const splittedModulePart = modulePart.split(Path.sep)
        .filter((part) => !!part); // remove empty strings

    let name;
    if (modulePart.indexOf(Path.sep + '@') === 0) { // detect scoped modules (ie. @owner/name)
        name = splittedModulePart[0] + '/' + splittedModulePart[1];
    }
    else {
        name = splittedModulePart[0];
    }

    const baseDir = pathToModule.slice(0, pathToModule.lastIndexOf(STR_NODE_MODULES) + STR_NODE_MODULES.length) + Path.sep + name;
    const relativePath = pathToModule.split(baseDir + Path.sep).pop();

    Logger.DEBUG(`Sqreen has identified the module ${name}`);

    return { name, baseDir, relativePath };
};
...
module.exports.scan = function (request, parent) {
    ...
    const isCore = pathToModule.indexOf(Path.sep) < 0; // FIXME: decide if we need to specially manipulate lib/internal in the core...
