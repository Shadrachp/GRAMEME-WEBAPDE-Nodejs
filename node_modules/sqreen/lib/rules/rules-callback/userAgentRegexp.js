...
const UARegexpCB = class {
    ...
    constructor(rule) {
        ...
        this.superMatcher = RegexpRule.getRegexpMatcherObject(rule.data.values.map((str) => str.replace('.*.', '').replace('.*', '')).join('|'));
        this.rule = rule;
        const self = this;
        this.pre = function (args) {

            if (args[0] === 'request' && args[1] && args[1].headers) {
                return self.executePre(args[1].headers['user-agent'], args);
            }
            return null;
        };
    }
