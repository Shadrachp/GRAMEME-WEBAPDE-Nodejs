...
const Run = class {
    ...
    constructor(cb, methods, rule) {
        ...
        this.method = function (args, value, _, selfObj, session) {

            return self.action(args, value, selfObj, session);
        };
        this.params = cb.slice(0, -1);
        this.data = this.rule.data || {};
    }

    run(params, selfObject) {
    ...
    bindThis(key, args, value, selfObj, session, req) {
        ...
        if (masterItem) {
            ...
            const sqreenMatches = { // TODO: lazy creation
                ...
                '#.cwd': () => process.cwd(),
                '#.request_params': () => Object.assign(request_params, session.body, session.query, session.cookies),
                '#.filtered_request_params': () => Object.assign(filtered_request_params, session.body, session.query)
            };
            if (session.__sqreen_lookup && session.__sqreen_lookup.hapi) {
                Object.assign(request_params, session.__sqreen_lookup.hapi.query, session.__sqreen_lookup.hapi.payload, session.__sqreen_lookup.hapi.state);
                Object.assign(filtered_request_params, session.__sqreen_lookup.hapi.query, session.__sqreen_lookup.hapi.payload);
                ...
                Object.assign(session.body, session.__sqreen_lookup.hapi.payload);
            }
    ...
    action(args, value, selfObj, session) {
        ...
        const result = this.run(params, selfObj); // yes, those methods can change the values of the arguments passed to the instrumented methods
