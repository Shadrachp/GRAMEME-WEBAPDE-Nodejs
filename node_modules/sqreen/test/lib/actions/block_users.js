...
describe('Actions', () => {
    ...
    describe('BLOCK USERS', () => {
        ...
        it('should add a user to block and block it', { plan: 8 }, (done) => {
            ...
            expect(record1.observed.sdk[1].name).to.equal('track');

            expect(record1.observed.sdk[1].args[0]).to.equal('sq.action.block_user');
            expect(record1.observed.sdk[1].args[1].properties).to.equal({ action_id: 999, output: { username:'toto', id: 42 } });
        ...
        it('should refuse malformed actions', { plan: 1 }, (done) => {
            ...
            expect(errors).to.equal([999, 1492]);
            done();
        });
    });
});
