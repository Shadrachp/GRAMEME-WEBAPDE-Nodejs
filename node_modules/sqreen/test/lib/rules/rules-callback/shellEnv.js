...
describe('ShellEnvCB', () => {
    ...
    describe('getEnv', () => {
        ...
        it('should find the env', { plan: 12 }, (done) => {
            ...
            const base = Object.keys(process.env).map((key) => ({ key, value: process.env[key] }));
            ...
            expect(GetEnv([null, [], { env: { a: 1 } }])).to.equal([{ key: 'a', value: 1 }]); // execFile(cmd, args, opt)
            expect(GetEnv([null, { env: { a: 1 } }])).to.equal([{ key: 'a', value: 1 }]); // execFile(cmd, opt)
            expect(GetEnv([null, [], { env: { a: 1 } }, () => {}])).to.equal([{ key: 'a', value: 1 }]); // execFile(cmd, args, opt, cb)
            const fakeArgs = [];
            fakeArgs.env = { a: 1 };
        ...
        it('should handle edge case', { plan: 1 }, (done) => {

            const base = Object.keys(process.env).map((key) => ({ key, value: process.env[key] }));

            expect(GetEnv([null, { hasOwnProperty: null }, () => {}])).to.equal(base); // exec(cmd, opt, cb)
            done();
        });
    });
    ...
    it('should detect a shellshock in arguments env variable passed as argument', { plan: 4 }, (done) => {
        ...
        expect(res.record.variable_name).to.equal('x');
        expect(res.record.variable_value).to.equal('bab');
        done();
    });
